=== uLogin - виджет авторизации через социальные сети ===
Contributors: lobovkin
Donate link: http://ulogin.ru/
Tags: ulogin, login, social, authorization
Requires at least: 2.9
Tested up to: 3.3.1
Stable tag: 1.8.3

Форма авторизации uLogin через социальные сети. Улучшенный аналог loginza.

== Description ==

uLogin — это инструмент, который позволяет пользователям получить единый доступ к различным Интернет-сервисам без необходимости повторной регистрации,
а владельцам сайтов — получить дополнительный приток клиентов из социальных сетей и популярных порталов (Google, Яндекс, Mail.ru, ВКонтакте, Facebook и др.)

== Installation ==

1. Распакуйте архив с плагином в папку `/wp-content/plugins/ulogin`
2. Активируйте плагин через меню 'Плагины' а Панели управления.
3. Если панель авторизации не появилась автоматически в форме комментария, пропишите в любом месте темы <?php echo ulogin_panel(); ?>.

== Frequently Asked Questions ==

= Нужно ли где-то регистрироваться, чтобы плагин заработал? =

Нет, плагин заработает сразу после установки!

= Панель не появилась автоматически в моей теме, что делать? =

Используйте код <?php echo ulogin_panel(); ?> для ручного вывода панели в любом месте темы.

== Screenshots ==

1. Форма комментирования

== Changelog ==

= 1.0 =
* Релиз.
= 1.1 =
* Траблы с SVN
= 1.2 =
* Исправлен js код.
* Добавлена функция ulogin_panel() для ручного вывода панели.
= 1.3 =
* Исправлена ошибка, возникающая, если email уже был в базе.
= 1.4 =
* Исправлена ошибка, возникающая, из-за динамического подключения скрипта
= 1.5 =
* Исправлена генерация пароля на wp_generate_password()
= 1.6 =
* Добавлена поддержка плагина Simplemodal Login Form
= 1.7 =
* Исправление ошибок, неограниченное количество сущностей uLogin на странице
= 1.8 =
* Добавлена страница настроек в административной панеле, добавлена панель на форму логина.
= 1.8.1 =
* Добавлена панель uLogin для формы регистрации;
* Расширен способ получения пользовательских данных от uLogin(curl или file_get_contents);
* В профиль пользователя корректно добавляется ссылка на страницу в соц. сети.
= 1.8.3 =
* Исправлена ошибка при работе с плагином обязательной авторизации ( http://ulogin.reformal.ru/proj/?ia=649892 )
* после авторизации на странице /wp-login.php теперь отправляет на главную
* исправление ошибок

== Upgrade Notice ==

= 1.0 =
Первый релиз

