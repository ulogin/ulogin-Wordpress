 ...
 */
include_once(substr(ABSPATH, 0, -1) . admin_url( $path = '', $scheme = 'relative').'includes/plugin.php');
...
/**
 * Получение данных с помощью HTTP API WordPress
 * @param string $url
 * @return bool|mixed|string
 */
...
function ulogin_check_token_error($u_user) {
	...
	if(isset($u_user['error'])) {
		...
		if($strpos) {
			wp_die(__('<b>Ошибка работы uLogin:</b></br></br>' . '<i>ERROR</i>: адрес хоста не совпадает с оригиналом') . sub($u_user['error'], (int)$strpos + 12), 'uLogin error', array('back_link' => true));
		}
		switch($u_user['error']) {
			case 'token expired':
...
function ulogin_get_user_photo($u_user, $user_id) {
	...
	$response = ulogin_get_response($file_url);
	if(!$response) {
		return false;
	}
...
function ulogin_registration_user($u_user, $in_db = 0, $policyWasAccepted = false) {
	...
	if(!$check_m_user && !$isLoggedIn) { // отсутствует пользователь с таким email в базе WP -> регистрация
		...
		if(!is_wp_error($user_id) && $user_id > 0 && ((bool)$uLoginOptions['new_user_notification'] === true)) {
...
function ulogin_validate_gravatar($email = '', $id = 0) {
	...
	return !is_wp_error($response) && (int)$response['response']['code'] === 200;
}
